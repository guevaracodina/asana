...
myYlims = [-40 100];  %change these values
for iChannels=1:nChannels+1
    subplot(nRow, nCol, iChannels)
    hold on
    if iChannels<=nChannels
    % stdErr = @(x) std(x)./numel(x);
    hCtl = shadedErrorBar(timeVector, 1e6*squeeze(HbOchannels.L(~isAfter,iChannels,:)),{@mean, stderror},':', 1);
    ...
    hPD = shadedErrorBar(timeVector, 1e6*squeeze(HbOchannels.L(isAfter,iChannels,:)),{@mean,stderror},'-', 1);
...
for iChannels=1:nChannels+1
    ...
    if iChannels<=nChannels
    % stdErr = @(x) std(x)./numel(x);
    hCtl = shadedErrorBar(timeVector, 1e6*squeeze(HbRchannels.L(~isAfter,iChannels,:)),{@mean, stderror},':', 1);
    hCtl.mainLine.Color = beforeColor;
    hCtl.edge(1).Color = beforeColor;
    hCtl.edge(2).Color = beforeColor;
	if ~isempty(hCtl.patch)
    ...
    hCtl.mainLine.LineWidth = 2;
    hPD = shadedErrorBar(timeVector, 1e6*squeeze(HbRchannels.L(isAfter,iChannels,:)),{@mean,stderror},'-', 1);
    hPD.mainLine.Color = afterColor;
    hPD.edge(1).Color = afterColor;
    hPD.edge(2).Color = afterColor;
    if ~isempty(hPD.patch)
